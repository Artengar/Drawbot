...
#Create Animation
def createAnimation(inputDirectory):
    ...
    while frameNr < len(folderContents):
        ...
        saveImage(exportFolder + currentName + ".gif")
        frameNr+=1
...
if os.path.isdir(os.path.expanduser(inputFolder + firstLevel[0])) == True:
    for item in firstLevel:
        ...
        newDrawing()
        createAnimation(inputDirectory)#Create SignIcons with EACH folder from the inputPath
##or handle no subfolders
        isFirstPage = False
else:
    inputDirectory = inputFolder
    createAnimation(inputDirectory)#Create SignIcons with the inputPath
